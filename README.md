...
**Внимание!** Вам понадобится **путь к папк**е, в которой будут храниться **.pdf файлы**, заранее **создайте** её и **скопируйти её полный путь**.
```bash
source ./setup.sh
```
...
server {
  ...
  location /pdfs {
    root <ПОЛНЫЙ ПУТЬ ДО ПАПКИ С PDF ФАЙЛАМИ, КОТОРЫЙ ВЫУКАЗЫВАЛИ КАК PDFS_DIR ВО ВРЕМЯ КОНФИГУРАЦИИ setup.sh>;
  }
}
```
